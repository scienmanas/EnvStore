import { OAuthGoogleClient } from "@/config/OAuth";
...
export async function GET(req: NextRequest) {
  ...
  // Remove any prevoius auth token
  if (authToken) {
  ...
  try {
    ...
    if (!payload)
      return NextResponse.json(
        { message: "Invalid/No ID Token" },
        { status: 400 }
      );
    // Extract user information from the payload
    if (!payload.email || !payload.name)
      return NextResponse.json(
        { message: "Required user information missing, oauth problem" },
        { status: 400 }
      );
